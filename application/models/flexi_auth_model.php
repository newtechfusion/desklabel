...
class Flexi_auth_model extends Flexi_auth_lite_model
{
		...
		foreach($user_account_cols as $key => $column)
		{
			if (isset($user_data[$column]))
			{
				...
				else
				{
					...
					if (isset($this->auth->tbl_col_user_account[$key]))
						$sql_update[$this->auth->tbl_col_user_account[$key]] = $user_data[$column];
					else
						$sql_update[$this->auth->tbl_custom_col_user_account[$key]] = $user_data[$column];
				}
	 ...
	 * @author Rob Hussey
	 */
	public function login($identity = FALSE, $password = FALSE, $remember_user = FALSE, $oauth_user_id = FALSE)
	{	
		// OAuth login start, added 02/06/2013, CBMc
		// If logging in via OAuth2 (ie Facebook), try loging in that way first
		if ($oauth_user_id !== FALSE):
			$sql_select = array(
				$this->auth->primary_identity_col, 
			...
			$sql_where = array('oauth_user_id' => $oauth_user_id);
	...
	public function send_email($email_to = NULL, $email_title = NULL, $data = NULL, $template = NULL)
	{
		...
		*/
		$this->load->model('email/mailgun');
		$this->mailgun->send($email_to, $this->auth->email_settings['reply_email'], $this->auth->email_settings['site_title'] ." ". $email_title,
